...
function openAuthModal(mode = 'signup', role = 'user', customSubtext = null) {
  ...
  // Administrator role is strictly restricted to login mode
  if (role === 'admin') {
    mode = 'login';
  }
    ...
    <div class="modal" style="max-width: 440px; border-radius: var(--radius-xl); padding: 32px; box-shadow: var(--shadow-xl);">
      <div style="text-align: center; margin-bottom: 20px;">
        ...
        <h3 id="auth-modal-title" style="font-size: 1.45rem; font-weight: 800; color: var(--gray-900); margin: 0 0 6px;">
          ${role === 'admin' ? 'Admin Control Gate 🔐' : (isSignup ? 'Create LifeLink Account' : 'Welcome Back')}
        </h3>
        ...
        <p style="font-size: 0.88rem; color: var(--text-secondary); margin: 0;">
          ${role === 'admin' ? 'Sign in to access admin control panel' : (isSignup ? 'Join thousands of registered blood donors saving lives' : 'Sign in to manage your donor profile & requests')}
        </p>
      </div>
      ...
      <form id="auth-form" onsubmit="${isSignup ? "handleAuthSignup(event, 'user')" : "handleAuthLogin(event, '" + role + "')"}" style="text-align: left;">
        ${isSignup ? `
      ...
      ${role === 'user' ? `
        <div style="margin-top: 20px; text-align: center; font-size: 0.85rem; color: var(--text-secondary);" id="auth-toggle-container">
          ${isSignup
            ? `Already have an account? <a href="#" onclick="switchAuthMode('login', 'user'); return false;" style="color: var(--accent); font-weight:700;">Log In</a>`
            : `Don't have an account? <a href="#" onclick="switchAuthMode('signup', 'user'); return false;" style="color: var(--accent); font-weight:700;">Sign Up Free</a>`
          }
        </div>
      ` : ''}
